...
function deriveFromTitle(title: string): Derived {
  ...
  if (/^team report:/i.test(title)) {
    return { kind: "team", start, end };
  }
  if (/^department report:/i.test(title)) {
    return { kind: "department", start, end };
  }
  if (/^project report:/i.test(title)) {
    return { kind: "project", start, end };
  }
  if (/personal report/i.test(title) && between) {
    return { kind: "personal", start, end };
  }
  return { kind: "unknown", start, end };
}
